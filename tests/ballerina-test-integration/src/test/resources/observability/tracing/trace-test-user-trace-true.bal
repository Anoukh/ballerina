...
service echoService bind listener {
    resourceOne (endpoint caller, http:Request clientRequest) {
        observe:Span span = observe:startSpan("uSpanOne", userTrace = true);
...
function callNextResource() returns (http:Response | error) {
    ...
    observe:Span span = observe:startSpan("uSpanTwo", userTrace = true);
    http:Response resp = check httpEndpoint -> get("/resourceTwo");
    _ = span.addTag("Allowed", "Successful");
    _ = span.finish();
    _ = span.addTag("Disallowed", "Unsuccessful");
    return resp;
}
